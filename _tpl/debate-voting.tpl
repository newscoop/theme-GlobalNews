        ...
        {{ if $gimme->default_article->defined }}
        	<div class="vote-box">
                ...
                {{ debate_form template="article.tpl" submit_button=false }}
                 ...
                 <a class="solid-button" onclick="$('#answer-{{ $gimme->debateanswer->number }}').attr('checked','checked');$(this).parents('form:eq(0)').submit(); return false;" href="javascript:void(0)">{{ $gimme->debateanswer->answer }}</a>
            ...
            {{ elseif $gimme->debate->is_current && !$gimme->user->logged_in }}<a class="solid-button debate-button" href="/auth">{{ #pleaseLoginVote# }}</a>{{ /if }}
        {{ /if }}
        </div>
    </div>
{{ /list_debates }} 
