...
<div class="audio-attachment">
  <h5><span class="glyphicon glyphicon-headphones"></span> {{ #listen# }}</h5><hr>
    <audio src="{{ uri options="articleattachment" }}" controls></audio><br>
    <a class="btn btn-default btn-xs" href="{{ uri options="articleattachment" }}">{{ #downloadAudioFile# }} | {{ $gimme->attachment->extension }}</a>
...
<div class="attachment">
    <h5><span class="glyphicon glyphicon-save"></span> {{ #attachment# }}</h5><hr>
    <a href="{{ uri options="articleattachment" }}" class="btn btn-default btn-xs">{{ #download# }} | {{ $gimme->attachment->file_name }} ({{ $gimme->attachment->size_kb }}kb)</a>
...
<div class="video-attachment"><!-- read http://diveintohtml5.org/video.html -->
  <h5 id="video-cont-label"><span class="glyphicon glyphicon-facetime-video"></span> {{ #watch# }}</h5><hr>
    ...
    {{foreach from=$videosources key=extension item=videoSource name=videoLoop}}
    <a href="{{ $videoSource }}" class="btn btn-default btn-xs">{{ #download# }} | {{ $extension }}</a>
    {{/foreach}}
