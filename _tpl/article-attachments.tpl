...
<div class="audio-attachment">
  <h5><i class="icon-headphones"></i> {{ #listen# }}</h5><hr>
    ...
    <a class="solid-button" href="{{ uri options="articleattachment" }}">{{ #downloadAudioFile# }} | {{ $gimme->attachment->extension }}</a>
...
<div class="attachment">
    ...
    <a href="{{ uri options="articleattachment" }}" class="solid-button">{{ #download# }} | {{ $gimme->attachment->file_name }} ({{ $gimme->attachment->size_kb }}kb)</a>
...
<div class="video-attachment"><!-- read http://diveintohtml5.org/video.html -->
  <h5 id="video-cont-label"><i class="icon-film"></i> {{ #watch# }}</h5><hr>
    ...
    {{foreach from=$videosources key=extension item=videoSource name=videoLoop}}
    <a href="{{ $videoSource }}" class="solid-button">{{ #download# }} | {{ $extension }}</a>
    {{/foreach}}
